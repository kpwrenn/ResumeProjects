 ...
 *
 * your name: Kevin Wrenn
 * Partner: Sarah Anthony
 */
...
public class Sudoku {    
    ...
    //Checks if row and column have a specific number 1-9
    //discussed with Sarah Anthony 
    ...
    public Sudoku() {
        ...
        this.columnHasValue = new boolean[9][10];
    }
    ...
    private boolean isValid(int i, int row, int col) { 
        return !this.valIsFixed[row][col] && !this.subgridHasVal[row/3][col/3][i] && !this.rowHasValue[row][i] && !this.columnHasValue[col][i];
    }
    ...
    private boolean solveRB(int n) {
        ...
        if (valIsFixed[row][col]) { 
            ...
            } //this returns somethin, ends method
         //discussed with Sarah Anthony 
        for (int i = 1; i <= 9; i++) { 
            if (isValid(i, row, col)) { 
                this.placeVal(i, row, col);
                // System.out.println("Current grid at n " + n);
                // printGrid();
                if (this.solveRB(n+1)) { 
                    return true;
                }
